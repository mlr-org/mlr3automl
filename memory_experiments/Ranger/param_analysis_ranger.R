...
ggplot(param_grid, aes(y = real_mib, x = log(nrow, base = 10))) + geom_point(aes(color = factor(num_trees))) + geom_smooth(method = "lm") + scale_color_viridis_d(direction = -1)
ggplot(param_grid, aes(y = real_mib, x = log(nrow, base = 10))) + geom_point(aes(color = factor(sample_fraction))) + geom_smooth(method = "lm") + scale_color_viridis_d(direction = -1) + facet_wrap(~num_trees)

ggplot(param_grid, aes(y = real_mib, x = num_trees)) + geom_point(aes(color = nrow), alpha = 0.7) + geom_smooth(method = "lm") + scale_color_viridis_c(direction = -1)
ggplot(param_grid, aes(y = real_mib, x = sample_fraction)) + geom_point(aes(color = nrow), alpha = 0.7) + geom_smooth(method = "lm") + scale_color_viridis_c(direction = -1)
ggplot(param_grid, aes(y = real_mib, x = nfeatures)) + geom_point(aes(color = nrow), alpha = 0.7) + geom_smooth(method = "lm") + scale_color_viridis_c(direction = -1)
...
glm_gamma = glm(rss_mib ~ nrow + nfeatures + num_trees + sample_fraction, data = param_grid, family = Gamma(link = "log"))
glm_gamma_log = glm(rss_mib ~ log(nrow) + log(nfeatures) + log(num_trees) + log(sample_fraction), data = param_grid, family = Gamma(link = "log"))
