...
- fix: The fastai learner now requires the Python `torch` package below
  version 2.14. Version 2.14 attaches logging handlers whose `stream` is
  a read-only property, which
  [`reticulate::py_capture_output()`](https://rstudio.github.io/reticulate/reference/py_capture_output.html)
  cannot redirect, so training failed with “property ‘stream’ of
  ’\_StderrHandler’ object has no setter”.
  [`install_python_learners()`](https://mlr3automl.mlr-org.com/dev/reference/install_python_learners.md)
  installs a compatible version when `"fastai"` is among the `learners`.
