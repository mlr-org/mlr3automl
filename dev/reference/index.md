# Package index

## Auto Learners

- [`LearnerClassifAuto`](https://mlr3automl.mlr-org.com/dev/reference/LearnerClassifAuto.md)
  : Classification AutoML Learner
- [`LearnerRegrAuto`](https://mlr3automl.mlr-org.com/dev/reference/LearnerRegrAuto.md)
  : Regression AutoML Learner

## Classification Auto Learners

- [`LearnerClassifAutoCatboost`](https://mlr3automl.mlr-org.com/dev/reference/LearnerClassifAutoCatboost.md)
  : Classification Gradient Boosted Decision Trees Auto Learner
- [`LearnerClassifAutoFastai`](https://mlr3automl.mlr-org.com/dev/reference/LearnerClassifAutoFastai.md)
  : Classification Fastai Auto Learner
- [`LearnerClassifAutoFTTransformer`](https://mlr3automl.mlr-org.com/dev/reference/LearnerClassifAutoFTTransformer.md)
  : Classification FT-Transformer Auto Learner
- [`LearnerClassifAutoGlmnet`](https://mlr3automl.mlr-org.com/dev/reference/LearnerClassifAutoGlmnet.md)
  : Classification GLM with Elastic Net Regularization Auto Learner
- [`LearnerClassifAutoKKNN`](https://mlr3automl.mlr-org.com/dev/reference/LearnerClassifAutoKKNN.md)
  : Classification k-Nearest-Neighbor Auto Learner
- [`LearnerClassifAutoLightGBM`](https://mlr3automl.mlr-org.com/dev/reference/LearnerClassifAutoLightGBM.md)
  : Classification LightGBM Auto Learner
- [`LearnerClassifAutoMLP`](https://mlr3automl.mlr-org.com/dev/reference/LearnerClassifAutoMLP.md)
  : Classification MLP Auto Learner
- [`LearnerClassifAutoRanger`](https://mlr3automl.mlr-org.com/dev/reference/LearnerClassifAutoRanger.md)
  : Classification Ranger Auto Learner
- [`LearnerClassifAutoResNet`](https://mlr3automl.mlr-org.com/dev/reference/LearnerClassifAutoResNet.md)
  : Classification ResNet Auto Learner
- [`LearnerClassifAutoSVM`](https://mlr3automl.mlr-org.com/dev/reference/LearnerClassifAutoSVM.md)
  : Classification SVM Auto Learner
- [`LearnerClassifAutoTabPFN`](https://mlr3automl.mlr-org.com/dev/reference/LearnerClassifAutoTabPFN.md)
  : Classification TabPFN Auto Learner
- [`LearnerClassifAutoXgboost`](https://mlr3automl.mlr-org.com/dev/reference/LearnerClassifAutoXgboost.md)
  : Classification XGBoost Auto Learner

## Regression Auto Learners

- [`LearnerRegrAutoCatboost`](https://mlr3automl.mlr-org.com/dev/reference/LearnerRegrAutoCatboost.md)
  : Regression Gradient Boosted Decision Trees Auto Learner
- [`LearnerRegrAutoFTTransformer`](https://mlr3automl.mlr-org.com/dev/reference/LearnerRegrAutoFTTransformer.md)
  : Regression FT-Transformer Auto Learner
- [`LearnerRegrAutoGlmnet`](https://mlr3automl.mlr-org.com/dev/reference/LearnerRegrAutoGlmnet.md)
  : Regression GLM with Elastic Net Regularization Auto Learner
- [`LearnerRegrAutoKKNN`](https://mlr3automl.mlr-org.com/dev/reference/LearnerRegrAutoKKNN.md)
  : Regression k-Nearest-Neighbor Auto Learner
- [`LearnerRegrAutoLightGBM`](https://mlr3automl.mlr-org.com/dev/reference/LearnerRegrAutoLightGBM.md)
  : Regression LightGBM Auto Learner
- [`LearnerRegrAutoMLP`](https://mlr3automl.mlr-org.com/dev/reference/LearnerRegrAutoMLP.md)
  : Regression MLP Auto Learner
- [`LearnerRegrAutoRanger`](https://mlr3automl.mlr-org.com/dev/reference/LearnerRegrAutoRanger.md)
  : Regression Ranger Auto Learner
- [`LearnerRegrAutoResNet`](https://mlr3automl.mlr-org.com/dev/reference/LearnerRegrAutoResNet.md)
  : Regression ResNet Auto Learner
- [`LearnerRegrAutoSVM`](https://mlr3automl.mlr-org.com/dev/reference/LearnerRegrAutoSVM.md)
  : Regression SVM Auto Learner
- [`LearnerRegrAutoTabPFN`](https://mlr3automl.mlr-org.com/dev/reference/LearnerRegrAutoTabPFN.md)
  : Regression TabPFN Auto Learner
- [`LearnerRegrAutoXgboost`](https://mlr3automl.mlr-org.com/dev/reference/LearnerRegrAutoXgboost.md)
  : Regression XGBoost Auto Learner

## Isolated Python Learners

- [`LearnerClassifFastaiIsolated`](https://mlr3automl.mlr-org.com/dev/reference/LearnerClassifFastaiIsolated.md)
  : Fastai Learner Isolated
- [`LearnerClassifTabPFNIsolated`](https://mlr3automl.mlr-org.com/dev/reference/LearnerClassifTabPFNIsolated.md)
  : TabPFN Learner Isolated
- [`LearnerRegrTabPFNIsolated`](https://mlr3automl.mlr-org.com/dev/reference/LearnerRegrTabPFNIsolated.md)
  : TabPFN Regressor Learner Isolated

## Auto Configuration

- [`Auto`](https://mlr3automl.mlr-org.com/dev/reference/Auto.md) : Auto
  Class
- [`AutoCatboost`](https://mlr3automl.mlr-org.com/dev/reference/AutoCatboost.md)
  : Catboost Auto
- [`AutoExtraTrees`](https://mlr3automl.mlr-org.com/dev/reference/AutoExtraTrees.md)
  : Extra Trees Auto
- [`AutoFastai`](https://mlr3automl.mlr-org.com/dev/reference/AutoFastai.md)
  : Fastai Auto
- [`AutoFTTransformer`](https://mlr3automl.mlr-org.com/dev/reference/AutoFTTransformer.md)
  : FTTransformer Auto
- [`AutoGlmnet`](https://mlr3automl.mlr-org.com/dev/reference/AutoGlmnet.md)
  : Glmnet Auto
- [`AutoKknn`](https://mlr3automl.mlr-org.com/dev/reference/AutoKknn.md)
  : Kknn Auto
- [`AutoLda`](https://mlr3automl.mlr-org.com/dev/reference/AutoLda.md) :
  Lda Auto
- [`AutoLightgbm`](https://mlr3automl.mlr-org.com/dev/reference/AutoLightgbm.md)
  : Lightgbm Auto
- [`AutoMlp`](https://mlr3automl.mlr-org.com/dev/reference/AutoMlp.md) :
  Mlp Auto
- [`AutoRanger`](https://mlr3automl.mlr-org.com/dev/reference/AutoRanger.md)
  : Ranger Auto
- [`AutoResNet`](https://mlr3automl.mlr-org.com/dev/reference/AutoResNet.md)
  : ResNet Auto
- [`AutoSvm`](https://mlr3automl.mlr-org.com/dev/reference/AutoSvm.md) :
  Svm Auto
- [`AutoTabpfn`](https://mlr3automl.mlr-org.com/dev/reference/AutoTabpfn.md)
  : Tabpfn Auto
- [`AutoXgboost`](https://mlr3automl.mlr-org.com/dev/reference/AutoXgboost.md)
  : Xgboost Auto

## Sugar Functions

- [`mlr_auto`](https://mlr3automl.mlr-org.com/dev/reference/mlr_auto.md)
  [`auto()`](https://mlr3automl.mlr-org.com/dev/reference/mlr_auto.md) :
  Dictionary of Auto Objects

## Dictionary

- [`mlr_auto`](https://mlr3automl.mlr-org.com/dev/reference/mlr_auto.md)
  [`auto()`](https://mlr3automl.mlr-org.com/dev/reference/mlr_auto.md) :
  Dictionary of Auto Objects

## Callbacks

- [`mlr3automl.encapsulation_daemon`](https://mlr3automl.mlr-org.com/dev/reference/mlr3automl.encapsulation_daemon.md)
  : Encapsulation Daemon Callback
- [`mlr3automl.initial_design_runtime`](https://mlr3automl.mlr-org.com/dev/reference/mlr3automl.initial_design_runtime.md)
  : Initial Design Runtime Limit Callback

## Installation

- [`install_python_learners()`](https://mlr3automl.mlr-org.com/dev/reference/install_python_learners.md)
  : Install Python Learners

## Package

- [`mlr3automl`](https://mlr3automl.mlr-org.com/dev/reference/mlr3automl-package.md)
  [`mlr3automl-package`](https://mlr3automl.mlr-org.com/dev/reference/mlr3automl-package.md)
  : mlr3automl: Automated Machine Learning for 'mlr3'
