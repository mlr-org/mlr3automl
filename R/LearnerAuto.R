...
LearnerClassifAuto = R6Class("LearnerClassifAuto",
  ...
  public = list(
    ...
    initialize = function(
      ...
      learner_ids = c("glmnet", "fnn", "lda", "naive_bayes", "nnet", "ranger", "svm", "xgboost")
      ...
      graph = po("removeconstants", id = "pre_removeconstants") %>>%
        ...
        gunion(list(
          ...
          # fnn
          po("imputehist", id = "fnn_imputehist") %>>% po("imputeoor", id = "fnn_imputeoor") %>>% po("encode", method = "one-hot", id = "fnn_encode") %>>% po("removeconstants", id = "fnn_post_removeconstants") %>>% lrn("classif.fnn", id = "fnn"),
          # lda
